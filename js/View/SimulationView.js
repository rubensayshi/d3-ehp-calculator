var SimulationView = Backbone.View.extend({
    ...
    viewToModel: function() {
        _.each(this.fieldMap, function(field, selector) {
            ...
            if ($fieldObj.is('span') || $fieldObj.is('td')) {
                ...
                this.model.set(field, parseInt($fieldObj.val()));
    ...
    modelToView: function() {
        ...
        _.each(alternatives, function(alt, alt_field) {
            ...
            ehp_change   = (ehp_change   > 0) ? ("+" + this.prepareVal(ehp_change))         : this.prepareVal(ehp_change);
    ...
    prepareVal: function(val, field) {
        if (field == 'armor_reduc' || field == 'resist_reduc' || field == '%') {
            val *= 100;
            val = this.roundNumber(val, 2);
            val = ""+val+"%";
            
            return val;
        } else {
            return this.roundNumber(val, 0);
        }
    },
